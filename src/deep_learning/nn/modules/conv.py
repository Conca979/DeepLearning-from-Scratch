...
def _im2col_indices(C_in: int, kH: int, kW: int,
        out_H: int, out_W: int, stride: int):
"""
Pre-compute the three index arrays (k, i, j) used by im2col and col2im.
Called once per ConvLayer during init() so they are not rebuilt each step.
...
def _im2col(x_pad: np.ndarray,
    k: np.ndarray, i: np.ndarray, j: np.ndarray,
    out_H: int, out_W: int) -> np.ndarray:
"""
Gather all receptive fields in one advanced-index operation.

x_pad  : (batch, C_in, H_pad, W_pad)
returns  col : (batch, C_in*kH*kW, out_H*out_W)
"""
...
def _col2im(col: np.ndarray, x_shape: tuple,
    k: np.ndarray, i: np.ndarray, j: np.ndarray,
    out_H: int, out_W: int, padding: int) -> np.ndarray:
"""
Transpose of im2col — scatter gradients back to the input spatial grid.
Duplicate contributions (overlapping windows) are accumulated with np.add.at.

col    : (batch, C_in*kH*kW, out_H*out_W)
x_shape: (batch, C_in, H, W)  — original un-padded shape
returns dx : (batch, C_in, H, W)
"""
...
col_r = col.reshape(batch, -1, out_H, out_W)   # (batch, C_in*kH*kW, out_H, out_W)
...
class InputLayer:
  """Feeds (batch, C, H, W) data into the network."""
...
# ══════════════════════════════════════════════════════════════════
#  ConvLayer  (im2col forward · optional Batch Norm · activation)
# ══════════════════════════════════════════════════════════════════

class Conv2D(Module):
  """
  Forward:  Z_raw = conv(input)  ──►  [BN(Z_raw)]  ──►  act(Z)
  Backward: chain rule through activation ► BN ► col2im / kernel grad

  layer_delta_term  =  dL / d(this layer's input)   shape: (batch, C_in, H, W)
  """
  ...
  def init(self) -> None:
    ...
    self._k, self._i, self._j = _im2col_indices(
      C_in, self.kH, self.kW, self.out_H, self.out_W, self.stride)
  ...
  def forward(self) -> None:
    ...
    x_pad = (np.pad(x, ((0,0),(0,0),(self.padding,)*2,(self.padding,)*2))
         if self.padding > 0 else x)
    col = _im2col(
      x_pad, self._k, self._i, self._j, self.out_H, self.out_W)
  ...
  def compute_delta_term(self, network, targets: np.ndarray) -> None:
    """
    Incoming: next_layer.layer_delta_term  =  dL / d(this layer's output A)
    Computes:
     ① dL/dZ   — chain through activation
     ② dL/dZ_raw  — chain through BN  (if enabled)
     ③ dW, db  — kernel / bias gradients  (stored for update_weights)
     ④ self.layer_delta_term  =  dL / d(input)  — propagated backward
    """
    ...
    self.layer_delta_term = _col2im(
      d_col, self.x_shape_cache,
      self._k, self._i, self._j,
      self.out_H, self.out_W, self.padding)

  def _bn_backward(self, d_out: np.ndarray) -> np.ndarray:
    """
    d_out : dL / d(BN output)  — shape (batch, C, H, W)
    Returns dL / d(BN input = Z_raw)

    Full derivation of Batch Norm backward:
     N     = batch * H * W
     dx̂   = d_out * γ
     dvar  = Σ  dx̂ · (x − μ) · (−½) · (σ²+ε)^{−3/2}
     dμ    = Σ  dx̂ · (−1/σ)  +  dvar · (−2/N) · Σ(x − μ)
     dx    = dx̂/σ  +  dvar · 2(x − μ)/N  +  dμ/N
    """
    Z    = self.Z_pre_bn
    mean = self.bn_mean
    var  = self.bn_var
    ...
    dvar  = (dx_hat * diff * (-0.5) * (var + eps) ** (-1.5)
         ).sum(axis=(0, 2, 3), keepdims=True)

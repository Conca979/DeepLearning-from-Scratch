...
class MaxPool2D(Module):
  """
  Forward  : strided-view trick   → one-shot max with no Python loops
  Backward : saved boolean mask routes gradients back to max positions;
        tied maxima share the gradient equally.

  layer_delta_term  =  dL / d(this layer's input)   shape: (batch, C, H, W)
  """
  ...
  def compute_delta_term(self, network, targets: np.ndarray) -> None:
    ...
    d_windows = (incoming[:, :, :, :, None, None]
           * (self.mask / count))             # (batch, C, out_H, out_W, p, p)
...
class Flatten(Module):
  """
  Forward : (batch, C, H, W)  →  (batch, C*H*W)
  Backward: reshapes incoming gradient back to conv shape.
  ...
  def compute_delta_term(self, network, targets: np.ndarray) -> None:
    # next Dense layer's layer_delta_term has shape (batch, C*H*W)
    self.layer_delta_term = (
      self.next_layer.layer_delta_term.reshape(self.conv_shape))

  def update_weights(self, lr: float) -> None:
    pass

# ══════════════════════════════════════════════════════════════════
#  DenseLayer  (fully-connected + optional Inverted Dropout)
# ══════════════════════════════════════════════════════════════════

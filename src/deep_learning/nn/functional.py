...
class LossFunction:
    @staticmethod
    def cc_loss(predicted_values: np.ndarray, targets: np.ndarray, derived: bool = False):
    ...
    @staticmethod
    def MSE(predicted_values: np.ndarray, targets: np.ndarray, derived: bool = False):
        batch = predicted_values.shape[0]
        if derived:

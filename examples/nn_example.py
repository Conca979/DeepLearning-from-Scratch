...
# =====================================================================
#  1.  Train the model (same setup as nn_run.py)
# =====================================================================
...
WEIGHTS_FILE = "../weights/nn_weights.npz"
...
if os.path.exists(WEIGHTS_FILE):
  ...
  accuracy = model.evaluate()
  print(f"Training complete in {elapsed:.1f}s  —  Test accuracy: {accuracy:.2f}%")
...
class DrawApp:
  ...
  def _build_ui(self):
    ...
    tk.Label(
      title_frame, text="✦  Digit Recogniser", font=self.font_title,
      fg=ACCENT, bg=BG_DARK
    ).pack()
    tk.Label(
      title_frame,
      text=f"Model accuracy: {accuracy:.2f}%  ·  Draw a digit below",
      font=self.font_small, fg=TEXT_MUTED, bg=BG_DARK,
    ).pack()
    ...
    tk.Label(digit_card, text="Prediction", font=self.font_label,
         fg=TEXT_MUTED, bg=PANEL_BG).pack()
    self.digit_label = tk.Label(
      digit_card, text="—", font=self.font_digit,
    ...
    self.conf_label = tk.Label(
      right, text="", font=self.font_small,
    ...
    for i in range(10):
      ...
      lbl = tk.Label(row, text=f"{i}", font=self.font_bar,
              fg=TEXT_MUTED, bg=BG_DARK, width=2, anchor="e")
      lbl.pack(side="left")
    ...
    self.predict_btn = tk.Button(
      btn_frame, text="⚡ Predict", font=self.font_btn,
    ...
    self.clear_btn = tk.Button(
      btn_frame, text="✕ Clear", font=self.font_btn,

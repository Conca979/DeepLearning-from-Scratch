...
class BasicNeuralNetwork:
  class HiddenLayer:
    ...
    def compute_delta_term(self, network: object, targets: np.ndarray) -> None:
  ...
  def _compute_delta_term(self, targets: np.ndarray) -> None:
  ...
  def fit_model(self) -> None:
    ...
    while not stop:
      ...
      # loop thought each batch from shuflled epoch
      for i in range(0, n_samples, self.batch):
        batch_indices = indices[i:i + self.batch]

        # Start propagation
        # the network takes batch as input
        ...
        self._compute_delta_term(targets= self.y_train[batch_indices])
    
        # Updating the smoothless of model loss
        if smoothed_loss is None:
          smoothed_loss = new_loss
        else:
          smoothed_loss = beta*smoothed_loss + (1-beta)*new_loss

        # Check stopping confition: exponentially weighted average

...
#  1.  Data
# =====================================================================
WEIGHTS_FILE = os.path.join(os.path.dirname(__file__), "..", "weights", "cnn_fashion_weights.npz")
...
x_train_raw = load_idx_images(train_img_path)
y_train_lbl = load_idx_labels(train_lbl_path)
x_test_raw = load_idx_images(test_img_path)
y_test_lbl = load_idx_labels(test_lbl_path)
...
PANEL_BG = "#1a1a2e"
CANVAS_BG = "#111122"
ACCENT = "#f43f5e"  # rose (different from MNIST version)
ACCENT_HOVER = "#fb7185"
TEXT_PRIMARY = "#e2e8f0"
TEXT_MUTED = "#94a3b8"
...
class DrawApp:
  ...
  def __init__(self, root: tk.Tk):
    self.root = root
    root.title("CNN — Fashion Recogniser")
    root.configure(bg=BG_DARK)
    root.resizable(False, False)

    self.font_title = tkfont.Font(family="Segoe UI", size=16, weight="bold")
    self.font_digit = tkfont.Font(family="Consolas", size=24, weight="bold")
  ...
  def _build_ui(self):
    ...
    tf.pack(fill="x")
    tk.Label(tf,
             text="✦  CNN Fashion Recogniser",
             font=self.font_title,
             fg=ACCENT,
             bg=BG_DARK).pack()
    tk.Label(tf,
             text=f"Conv + BN + Dropout  ·  Accuracy: {accuracy:.2f}%",
    ...
    for i, cls_name in enumerate(FASHION_CLASSES):
      ...
      # Determine short name for space reasons if necessary
  ...
  def _on_predict(self):
    ...
    # Centre drawing inside 20×20 box within 28×28 canvas
    ...
    arr = np.asarray(final, dtype=np.float32).reshape(1, 1, MNIST_PX,
                                                      MNIST_PX) / 255.0
    ...
    self.class_label.config(text=FASHION_CLASSES[best])
  ...
  def _draw_bars(self, probs, best):
    for i, p in enumerate(probs):
      ...
      bar.create_rectangle(0,
                           0,
                           fw,
                           self.BAR_H,
                           fill=SUCCESS if i == best else BAR_FILL,
                           outline="")

...
class ModelSetup:
  ...
  def __init__(self, trainingSet, testSet, initWeights, epsilon, learningRate,
               modelDegree, iterationLogTrigger):
  ...
  def polyTransform(self):
    self.polyEngine = PolyFeatureTransform(self.xTrain.shape[1],
                                           degree=self.degree)
    newInput = []
...
class BasicLinearRegression(ModelSetup):  # gradient decsent approach
  ...
  def __init__(self,
    ...
    super().__init__(trainingSet, testSet, initWeights, epsilon, learningRate,
                     modelDegree, iterationLogTrigger)
  ...
  def modelValidating(self):
    ...
    predictedOutComes = np.apply_along_axis(self.predict,
                                            axis=1,
                                            arr=self.xTest)
  ...
  def fitModel(self, log=None, saveModelState=None):
    ...
    while True:
      ...
      if self.logTrigger > 0 and self.iterationCount % self.logTrigger == 0:
        print(
            f"||| {self.iterationCount}\t iterations passed; model'current cost: {currentCost}"
        )
        if log:
          with open(log, mode='a') as f:
  ...
  def showModel(
      ...
      graph=None,
      logScale=True
  ):  # graphType = "Prediction Accurary" by default, "Cost vs Iteration" for any passed value
    if graph is None:
      ...
      plt.plot([lowerBound, upperBound], [lowerBound, upperBound],
               c='b',
               alpha=0.5)
    else:
...
class BasicLogisticRegression(ModelSetup):  # grandient ascent approach
  ...
  def computeLogLikelihood(self, predictions):
    ...
    return sum(self.yTrain * np.log(predictions) +
               (1 - self.yTrain) * np.log(1 - predictions))
  ...
  def fitModel(self, log=None, saveModelState=None):
    ...
    if log:
      ...
      with open(log, mode='a') as f:
        f.write(
            f"""["Weights"], current log likelihood, shifting in Llh \n {self.weights}, {curLlh}, 0 \n"""
        )
    ...
    while True:
      ...
      if self.logTrigger > 0 and self.iterationCount % self.logTrigger == 0:
        print(
            f"||| {self.iterationCount}\t iterations passed; model'current log-likelihood: {curLlh}"
        )
        if log:
          with open(log, mode='a') as f:
  ...
  def _modelEvaluation(self):
    predictions = np.array(
        [self.predict(inputData) for inputData in self.xTest])
    ...
    falsePos = sum(1 for a, p in zip(actLabels, preLabels)
                   if a == 0 and p == 1)
    falseNeg = sum(1 for a, p in zip(actLabels, preLabels)
                   if a == 1 and p == 0)

    # F-score components
    precision = truePos / (truePos + falsePos) if (truePos +
                                                   falsePos) > 0 else 0
    recall = truePos / (truePos + falseNeg) if (truePos + falseNeg) > 0 else 0
    specificity = trueNeg / (trueNeg + falsePos) if (trueNeg +
                                                     falsePos) > 0 else 0
  ...
  def showConfusionMatrix(self):
    ...
    # confusion matrix
    cmData = np.array([[m['tp'], m['fn']], [m['fp'], m['tn']]])
  ...
  def showTestTable(self):
    predictedProbabilities = np.apply_along_axis(self.predict,
                                                 axis=1,
                                                 arr=self.xTest)
    predictedLabels = [(1 if val >= 0.5 else 0)
                       for val in predictedProbabilities]
    tableData = []
    for i in range(len(self.testSet)):
      isCorrect = "Yes" if predictedLabels[i] == self.testSet[i][-1] else "No"
      tableData.append([
          self.testSet[i][-1],
          round(predictedProbabilities[i], 7), predictedLabels[i], isCorrect
      ])

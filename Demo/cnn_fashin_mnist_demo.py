...
class DrawApp:
  ...
  PANEL_BG     = "#1a1a2e"
  CANVAS_BG    = "#111122"
  ACCENT       = "#f43f5e"   # rose — distinct from the MNIST cyan
  ACCENT_HOVER = "#fb7185"
  TEXT_PRIMARY = "#e2e8f0"
  TEXT_MUTED   = "#94a3b8"
  ...
  def _build_ui(self):
    ...
    for i, cls_name in enumerate(self.FASHION_CLASSES):
  ...
  def _on_predict(self):
    ...
    # Build CNN input: (1, 1, 28, 28) float32 in [0, 1]
    arr   = np.asarray(final, dtype=np.float32).reshape(1, 1, self.MNIST_PX, self.MNIST_PX) / 255.0
    probs = model.predict(arr)[0]   # shape (10,)
    best  = int(np.argmax(probs))

    self.class_label.config(text=self.FASHION_CLASSES[best])
...
#  Main — requires pre-trained weights (run training/cnn_train_fashion_mnist.py first)
if __name__ == "__main__":
  ...
  if not os.path.exists(WEIGHTS_FILE):
    ...
    print("  -> Run  training/cnn_train_fashion_mnist.py  first to generate weights.")
    sys.exit(1)

  act = ActivationFunction
  layers = [
    InputLayer(None, input_shape=(-1, 1, 28, 28)),
  ...
  model = Network(layers=layers, loss_func=LossFunction.cc_loss, learning_rate=0.1)

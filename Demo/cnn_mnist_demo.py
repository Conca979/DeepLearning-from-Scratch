...
class DrawApp:
  ...
  TEXT_PRIMARY = "#e2e8f0"
  TEXT_MUTED   = "#94a3b8"
  SUCCESS      = "#a855f7"   # purple for predicted digit
  BAR_BG       = "#2a2a40"
  BAR_FILL     = "#06b6d4"
  ...
  def __init__(self, root: tk.Tk, accuracy: float = 0.0):
    self.root     = root
...
#  Main — requires pre-trained weights (run training/cnn_train_mnist.py first)
if __name__ == "__main__":
  ...
  if not os.path.exists(WEIGHTS_FILE):
    ...
    print("  -> Run  training/cnn_train_mnist.py  first to generate weights.")
    sys.exit(1)
